...
```{r}
# User input to choose sample of interest to view ----
isolate2 <- reactive(input$isolate)

selectizeInput("isolate2",
               h4("Select or search for an isolate:"),
               choices = isolates,
               options = list(maxItems = 1))
```
...
```{r}
# Load in the current isolate the user has chosen to view so it can be used for "recentrifuge_html_path"
selected_isolate2 <- reactive(input$isolate2)
read_dist_path <- reactive(paste0("path_to_results/", selected_isolate2(), "_all_reads.fq_read_length_distrabution_plot.png"))
