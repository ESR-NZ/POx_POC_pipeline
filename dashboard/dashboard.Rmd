...
# Read top classification results data
top_class <- read.csv(paste0(results_dir, "classification_results.csv"))

# Get only the rows with 
...
```{r}
tophit <- reactive(top_class %>%
  filter(Barcode == selected_isolate()) %>%
  pull(Taxon1) %>%
  # Deal with multiple rows of data for the same isolate/sample
  unique() %>%
  # Italics
  em())
